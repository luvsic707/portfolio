...
PROJECTS = Path(__file__).resolve().parent.parent / "src" / "content" / "projects"
...
def process(project_dir: Path) -> str | None:
    ...
    for old in target.glob("*.jpg"):
        if re.match(r"^(cover|[a-z0-9-]+-\d{2})\.jpg$", old.name):
            old.unlink()
    ...
    if cover_dir.is_dir():
        picks = images_in(cover_dir)
        if picks:
            if convert(picks[0], target / "cover.jpg", MAX_EDGE_FULL):
                generated.append("cover.jpg")
                if re.search(r"^#?\s*cover:", front, flags=re.M):
                    front = re.sub(r"^#?\s*cover:.*$", "cover: ./cover.jpg", front, flags=re.M)
                    front = re.sub(r"^#?\s*coverAlt:.*$",
                                   f"coverAlt: {project_dir.name} key visual", front, flags=re.M)
                else:
                    front = front.rstrip() + "\ncover: ./cover.jpg\n"

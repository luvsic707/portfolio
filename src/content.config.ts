  ...
  schema: ({ image }) =>
    z.object({
      ...
      /* ---- 这一页的纸是什么白 ----
         ...
         dark：整页反过来，近黑的纸配浅墨。给素材本身就长在黑底上的
         项目 —— 九命的人设、界面件、印记四角都是纯黑，摊在惨白纸上
         每张都留一个方块边。注意：白底的图（比如原型录屏、流程图）
         在暗底上会变成一块块亮方，上这个之前先看一眼素材。

         只改 .sheet 的背景和墨线 token，不覆盖 --paper 变量 ——
         那个变量在深色带里是当**文字颜色**用的，覆盖了字会跟着变。 */
      ground: z.enum(['paper', 'white', 'dark']).optional(),
